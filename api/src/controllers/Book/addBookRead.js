...
export const addBookRead = async (req, res) => {
  try {
    ...
    if (!idGoogle || !userId) {
      return res.status(400).json({
        message: 'Se requiere el libro y el usuario para agregar a la lista de libros leídos',
      });
    }
    ...
    if (user.booksRead) {
      var bookIndex = user.booksRead.indexOf(savedBook._id);
    }
    ...
    user.booksRead.push(savedBook._id);
    await user.save();
